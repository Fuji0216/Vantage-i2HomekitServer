...
function SetServices(currentAccessory, services) {
    ...
    let head = currentAccessory.Platform.GenerialManager.New(
        currentAccessory.content.BLIND,
        apLoader,
        customContainer);

    currentAccessory.res = {};
    currentAccessory.res.VID = currentAccessory.content.BLIND;
    ...
    currentAccessory.res.APLoader.AddState("CurrentPosition", CP_Characteristic, currentAccessory.res.Const.InitValue);
    /* CurrentPosition Get */
    CP_Characteristic.on('get', function (callback) {
        ...
        //送出指令
        VantageCommand.Command.GETBLIND(
            currentAccessory.res.VID,
            currentAccessory.res.Client,
            currentAccessory.res.APLoader);

        setTimeout(function () {
            let level = currentAccessory.res.AP2I.GetRemoteState(currentAccessory.res.APLoader, "CurrentPosition");
    ...
    currentAccessory.res.APLoader.AddState("TargetPosition", TP_Characteristic, currentAccessory.res.Const.InitValue);
    /* TargetPosition Get */
    TP_Characteristic.on('get', function (callback) {
        //非被動式，不須詢問狀態

        setTimeout(function () {
            let level = currentAccessory.res.AP2I.GetRemoteState(currentAccessory.res.APLoader, "TargetPosition");
            ...
            let promise = new Promise(function (resolve, reject) {
                ...
                let timeoutID = setInterval(function () {
                    ...
                    //判斷當上一個需要執行的Promise已經結束
                        ...
                        currentAccessory.res.Variable.targetValue = _lockTarget;
                        //    (pluginAccessory.res.Variable.lastTargetValue == 100 && _lockTarget == 100) ? 150 :
                        //        (pluginAccessory.res.Variable.lastTargetValue == 0 && _lockTarget == 0) ? -50 : _lockTarget;

                        // 用於紀錄上一次動作的數值
                        //pluginAccessory.res.Variable.lastTargetValue = _lockTarget;
                ...
                let targetValue = currentAccessory.res.Variable.targetValue;
                if (targetValue == 100 || targetValue == 0) {
                    VantageCommand.Command.BLIND(
                        currentAccessory.res.VID,
                        (targetValue == 100) ? 1 : (targetValue == 0) ? 2 : 0,
                        currentAccessory.res.Client,
                        currentAccessory.res.APLoader);
                }
                else {
                    VantageCommand.Command.BLINDPOS(
                        currentAccessory.res.VID,
                        targetValue,
                        currentAccessory.res.Client,
                        currentAccessory.res.APLoader);
